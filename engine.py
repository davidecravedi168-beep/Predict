...
DISPLAY_TICKER = {"ES=F":"ES","NQ=F":"NQ"}
...
def historical_edge(direction, sector, vix_now):
    edge = 50.0
    reasons = []
    if direction == "LONG":
        edge += 8
        reasons.append("LONG stronger than SHORT in first 5y backtest")
    else:
        edge -= 10
        reasons.append("SHORT weaker in first 5y backtest")

    if vix_now >= 30:
        edge += 15
        reasons.append("VIX>=30 strongest historical regime")
    elif vix_now >= 20:
        edge -= 5
        reasons.append("VIX 20-30 historically weak")
    else:
        edge -= 2
        reasons.append("VIX<20 slightly weak overall")
    ...
    edge += adj
    if sector == "INDEX_FUTURES":
        reasons.append("ES/NQ neutral pending dedicated futures backtest")
    elif adj > 0:
        reasons.append(f"{sector} historically stronger")
    elif adj < 0:
        reasons.append(f"{sector} historically weaker")
    return float(np.clip(edge, 0, 100)), reasons
...
def score_asset(ticker, sector, data):
    ...
    edge_score, edge_reasons = historical_edge(direction, sector, vix_now)
    if edge_score >= 65:
        hist_adj = 0.01
    elif edge_score >= 50:
        hist_adj = 0.00
    ...
    calibrated_confidence = float(np.clip(confidence + hist_adj,0.50,0.92))
    ...
    return {
        ...
        "model_price": round(float(s.iloc[-1]),4),
...
def main():
    ...
    quote_map, market_data_at = latest_quotes([x["ticker"] for x in rows])
    for x in rows:
        qp = quote_map.get(x["ticker"])
        x["quote_price"] = round(float(qp),4) if qp is not None else None
        x["entry_price"] = round(float(qp),4) if qp is not None else x["price"]
