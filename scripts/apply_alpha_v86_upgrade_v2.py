#!/usr/bin/env python3
"""Fail-closed, idempotent Alpha Engine v8.6 episode-ledger migration."""
...
def patch_engine(text):
    ...
    for old, new, label in [
        ...
        text, c = replace_unique(text, old, new, label)
        changed |= c

    # Patch only the Alpha-prediction loop; the same short guard also appears in
    # the independent external-model ledger and must remain HIT/MISS-only.
    ...
    text, c = replace_one(text, old_skip, new_skip, 'terminal outcomes')
    changed |= c
    ...
    text, c = replace_unique(text, old_resolved, new_resolved, 'clean-lineage resolved stats')
    changed |= c

    new_add = '''def _episode_key_from_parts(ticker, direction, horizon, setup_family):
...
'''
    text, c = replace_regex(
        text,
        r'def add_prediction\(mem, sig\):.*?\n\ndef _segment',
        new_add + '\n\ndef _segment',
        'episode-aware add_prediction',
        marker='def migrate_prediction_episodes(mem):',
    )
    changed |= c
    ...
    mem["stats"] = {
        ...
        "resolved": len(resolved),'''
    text, c = replace_unique(text, old_stats, new_stats, 'clean stats counters')
    changed |= c

    old_edge = '''    edge, edge_reasons = historical_edge(mem, asset_class, direction, horizon)
    reasons = []'''
    new_edge = '''    model_completeness_fields = {
    ...
    reasons = []'''
    text, c = replace_unique(text, old_edge, new_edge, 'model completeness')
    changed |= c

    old_return = '''        "data_quality_score": data_quality,
        "data_quality_fields": expected,
        "model_consensus": round(consensus, 3),'''
    new_return = '''        "data_quality_score": data_quality,
        "data_quality_fields": expected,
        ...
        "model_consensus": round(consensus, 3),'''
    text, c = replace_unique(text, old_return, new_return, 'completeness output')
    changed |= c
    return text, changed
...
function compactJournalEpisodes(rows){
  ...
  for(const z of sorted){
    ...
    if(prev){
      ...
      prev.episode_id=prev.episode_id||prev.id;prev.episode_key=key;
      ...
      if(safeNum(z.last_price,null)!=null)prev.last_price=z.last_price;
      ...
      prev.duplicate_snapshots=safeNum(prev.duplicate_snapshots,0)+1;continue;
    }
    z.episode_id=z.episode_id||z.id;z.episode_key=key;
  ...
  for(const group of byTicker.values()){
    if(group.length<2)continue;group.sort((a,b)=>safeNum(a.ts,0)-safeNum(b.ts,0));const newest=group[group.length-1];
  ...
  return out.sort((a,b)=>safeNum(a.ts,0)-safeNum(b.ts,0));
    ...
    text, c = replace_unique(text, anchor, insert, 'journal compactor')
    changed |= c

    # Scope the read migration to getJournal so unrelated array helpers are untouched.
    old_get = "return Array.isArray(x)?x:[];\n  }catch(e){return []}\n}\nfunction saveJournal"
    new_get = "return Array.isArray(x)?compactJournalEpisodes(x):[];\n  }catch(e){return []}\n}\nfunction saveJournal"
    text, c = replace_unique(text, old_get, new_get, 'journal read compaction')
    changed |= c

    text, c = replace_unique(text, "if(['TARGET2','TARGET1','STOP','AMBIGUO'].includes(z.status)) continue;", "if(JOURNAL_TERMINAL.has(z.status)) continue;", 'journal terminal skip')
    changed |= c
    text, c = replace_unique(text, "if(z.status!=='OPEN') z.resolved_at=z.observed_at;", "if(['TARGET1','TARGET2','STOP','AMBIGUO'].includes(z.status)) z.resolved_at=z.observed_at;", 'journal resolved timestamp')
    changed |= c
    text, c = replace_unique(text, "const payload={format:'ALPHA-DECISION-JOURNAL',version:'8.5',exported_at:new Date().toISOString(),journal:j};", "const payload={format:'ALPHA-DECISION-JOURNAL',version:'8.6',exported_at:new Date().toISOString(),journal:compactJournalEpisodes(j)};", 'journal export schema')
    changed |= c
    text, c = replace_unique(text, "a.download='alpha-v8-decision-journal-'+", "a.download='alpha-v86-decision-journal-'+", 'journal export filename')
    changed |= c
    text, c = replace_unique(text, "status:['OPEN','OBSERVE_ONLY','TARGET1','TARGET2','STOP','AMBIGUO'].includes(z.status)?z.status:'OPEN'", "status:['OPEN','OBSERVE_ONLY','TARGET1','TARGET2','STOP','AMBIGUO','SUPERSEDED','EXPIRED'].includes(z.status)?z.status:'OPEN'", 'import statuses')
    changed |= c
    text, c = replace_unique(text, "saveJournal([...map.values()].sort((a,b)=>safeNum(a.ts,0)-safeNum(b.ts,0))); updateJournalObservations(); renderJournal();", "saveJournal(compactJournalEpisodes([...map.values()])); updateJournalObservations(); renderJournal();", 'import compaction')
    changed |= c

    snapshot_new = '''function snapshotSignals(){
  if(!D)return;let original=getJournal(),j=compactJournalEpisodes(original),changed=JSON.stringify(j)!==JSON.stringify(original);
  ...
  for(const x of pool){
    ...
    if(activeSame){activeSame.last_seen_at=D.market_data_at||D.updated_at||new Date().toISOString();activeSame.last_model_version=D.model_version||x.model_version||activeSame.model_version;activeSame.latest_confidence=safeNum(de.final_confidence,activeSame.latest_confidence);changed=true;continue}
    const priorActive=j.filter(z=>!JOURNAL_TERMINAL.has(z.status)&&z.ticker===x.ticker),episodeId='EP_'+x.ticker+'_'+x.direction+'_'+now;
  ...
  if(changed)saveJournal(compactJournalEpisodes(j));
    ...
    text, c = replace_unique(text, old_box, new_box, 'completeness UI')
    changed |= c
    ...
    text, c = replace_unique(text, old_test, new_test, 'system test episode guard')
    changed |= c

    for old, new in [("j.length+' snapshot'", "j.length+' episodi'"), ("'alpha_last_data_v85'", "'alpha_last_data_v86'"), ("./data/latest.json?build=v85edgecore", "./data/latest.json?build=v86episodeledger")]:
        if old in text:
...
def main():
    ...
    for marker in ['MODEL_VERSION = "8.6.0-episode-ledger-cost-aware"', 'def migrate_prediction_episodes(mem):', 'UPDATED_ACTIVE_EPISODE', '"model_completeness_score": model_completeness_score', '"schema_version": "8.6"']:
        if marker not in engine2: raise RuntimeError(f'engine verification failed: {marker}')
    for marker in ['function compactJournalEpisodes(rows)', 'Journal Episode Guard', "version:'8.6'", 'v86episodeledger', 'Completezza modello']:
        if marker not in index2: raise RuntimeError(f'index verification failed: {marker}')
    if '<12*3600*1000' in index2: raise RuntimeError('legacy 12-hour duplicate gate still present')
